    ...
    <div class="row">
        <div class="col">
            <small class="float-right text-muted">
                <strong>{{ __('labels.frontend.opportunity.internships.tabs.content.overview.created_at') }}:</strong> {{ timezone()->convertToLocal($internship->created_at) }} ({{ $internship->created_at->diffForHumans() }})@if (isset($internship->createdByUser)) by {{ $internship->createdByUser->full_name }} ({{ $internship->createdByUser->id }})@endif,
                <strong>{{ __('labels.frontend.opportunity.internships.tabs.content.overview.last_updated') }}:</strong> {{ timezone()->convertToLocal($internship->updated_at) }} ({{ $internship->updated_at->diffForHumans() }})@if (isset($internship->createdByUser)) by {{ $internship->updatedByUser->full_name }} ({{ $internship->updatedByUser->id }})@endif
                @if ($internship->trashed())
                    <strong>{{ __('labels.frontend.opportunity.internships.tabs.content.overview.deleted_at') }}:</strong> {{ timezone()->convertToLocal($internship->deleted_at) }} ({{ $internship->deleted_at->diffForHumans() }})
                @endif
            </small>
        </div><!--col-->
    </div><!--row-->
