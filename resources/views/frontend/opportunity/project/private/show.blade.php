    ...
    <div class="row">
        <div class="col">
            <small class="float-right text-muted">
                <strong>@lang('labels.frontend.opportunity.projects.tabs.content.overview.created_at'):</strong> {{ timezone()->convertToLocal($project->created_at) }} ({{ $project->created_at->diffForHumans() }})@if (isset($project->createdByUser)) by {{ $project->createdByUser->full_name }} ({{ $project->createdByUser->id }})@endif,
                <strong>@lang('labels.frontend.opportunity.projects.tabs.content.overview.last_updated'):</strong> {{ timezone()->convertToLocal($project->updated_at) }} ({{ $project->updated_at->diffForHumans() }})@if (isset($project->createdByUser)) by {{ $project->updatedByUser->full_name }} ({{ $project->updatedByUser->id }})@endif
                @if ($project->trashed())
                    <strong>@lang('labels.frontend.opportunity.projects.tabs.content.overview.deleted_at'):</strong> {{ timezone()->convertToLocal($project->deleted_at) }} ({{ $project->deleted_at->diffForHumans() }})
                @endif
            </small>
        </div><!--col-->
    </div><!--row-->
