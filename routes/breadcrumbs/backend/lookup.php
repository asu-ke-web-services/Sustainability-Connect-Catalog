<?php

require __DIR__.'/lookup/affiliation.php';
require __DIR__.'/lookup/attachment_type.php';
require __DIR__.'/lookup/budget_type.php';
require __DIR__.'/lookup/category.php';
require __DIR__.'/lookup/keyword.php';
require __DIR__.'/lookup/opportunity_review_status.php';
require __DIR__.'/lookup/opportunity_status.php';
require __DIR__.'/lookup/opportunity_type.php';
require __DIR__.'/lookup/organization_status.php';
require __DIR__.'/lookup/organization_type.php';
require __DIR__.'/lookup/relationship_type.php';
require __DIR__.'/lookup/student_degree_level.php';
require __DIR__.'/lookup/user_type.php';
